---
name: modern-python
description: How to write and review modern Python (3.13+) the way a senior platform engineer does — correctness enforced by the type checker and linter, not by convention or runtime checks. Use whenever writing, refactoring, reviewing, or designing Python code, a data model, a config object, or a public function/class signature, including async/concurrent code and test strategy — even if the user just says "write a function that..." or pastes Python for feedback, and even for a "quick script." Covers parse-don't-validate, making illegal states unrepresentable, sentinel objects for overloaded None, exhaustiveness with assert_never, TypeIs/TypeGuard vs cast, avoiding getattr/hasattr/setattr, dataclasses vs Pydantic vs NamedTuple, immutability and shared mutable state, structured concurrency (asyncio.TaskGroup) and threading, what still needs a test, declarative control flow, ergonomic API design, and the uv/ruff/ty toolchain. This is a hub — it stays short and links to references/*.md for the topic actually in play; consult those files rather than trying to hold the whole philosophy in context at once.
---
